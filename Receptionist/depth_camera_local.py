...
def filter_detections(detections, depth_point_cloud):
    ...
    for detection in detections:
        if detection[0] == 'person' and sum([(v / 1000) ** 2 for v in depth_point_cloud[int(detection[2][1])][int(detection[2][0])]]) ** 0.5 <= 5:
            persons.append(detection)
            new_detection.append(detection)
        elif detection[0] == 'chair' and sum([(v / 1000) ** 2 for v in depth_point_cloud[int(detection[2][1])][int(detection[2][0])]]) ** 0.5 <= 5:
            chairs.append(detection)
            new_detection.append(detection)
        elif detection[0] == 'sofa' and sum([(v / 1000) ** 2 for v in depth_point_cloud[int(detection[2][1])][int(detection[2][0])]]) ** 0.5 <= 5:
            sofa = detection
            new_detection.append(detection)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        capture = camera.get_capture()
        detections = filter_detections(image_detection_original_no_image(capture.color, network, class_colors, 0.3, fx, fy), capture.transformed_depth_point_cloud)
        print(detections)
        if is_debug:
            ...
            if cv2.waitKey(1) in [ord('Q'), ord('q')]:
                ...
                exit(0)
        if len(chairs) != 2:
            print(colored("The number of chairs is not exactly 2!", 'bright yellow'))
            continue
        if sofa == '':
        ...
        if flag and index != -1:
            print("Chair!")
            print(colored("Empty position index is {}!".format(index)))
            continue
